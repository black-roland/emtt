...
forwarded-to-telegram = Сообщение отправлено в Telegram (от { $from }):
  { $message }
...
received-text-msg = Получено текстовое сообщение от { $from } (ID: { $id }):
  { $text }
...
forwarded-to-webhook = Сообщение отправлено в вебхук (от { $from }):
  { $message }
